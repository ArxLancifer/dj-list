...
app.use(cors({
    origin: ["https://memotrack-app.onrender.com","http://localhost:3000"], // allow to server to accept request from different origin
...
app.post("/gatekeeper", async (req,res)=>{
    ...
    try {
        ...
        if(userIsAuth){
           ...
           return res.json({id:userIsAuth._id, email:userIsAuth.email, name:userIsAuth.username, favoriteLists:userFavoriteLists, isAuth:true, accountSince:userFavoriteLists.createdAt, userimage:userIsAuth.userimage});
        }else{
